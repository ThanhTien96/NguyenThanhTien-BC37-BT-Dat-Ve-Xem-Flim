...
const reducer = (state = initialState, action) => {
    switch(action.type){
        case 'CHOOSE_CHAIR':{
            const selectedChair = [...state.chairSelectedList];
            const findItem = selectedChair.findIndex(item => item.soGhe === action.payload.soGhe);

            if(findItem !== -1){
                selectedChair.splice(findItem, 1);
            }else {
                selectedChair.push(action.payload);
            };

            return {...state, chairSelectedList: selectedChair}
        }
        case 'PAY':{
            const data = state.chairList.map(item => {
                const listChair = item.danhSachGhe.map(chair => {
                ...
                return {
                    ...item,
                    danhSachGhe: listChair,
                }
            })

            return{
                ...state, chairList: data, chairSelectedList: []
            }
        }
        default:
            return state;
